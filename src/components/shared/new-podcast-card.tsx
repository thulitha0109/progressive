...
export function NewPodcastCard({
  ...
  const handleCardClick = () => {
    ...
    if (isCurrentTrack) {
      ...
      // @ts-expect-error - PodcastItem is compatible with Track for playback
  ...
  return (
    ...
    >
      ...
      <div className="relative w-28 sm:w-40 h-full shrink-0 overflow-hidden rounded-l-md rounded-r-none shadow-lg isolate ring-1 ring-white/10 ring-inset">
        ...
        {isUpcoming && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/40 z-20 transition-all duration-300 backdrop-blur-[2px]">
      ...
      <div className="flex flex-col justify-between h-full min-w-0 flex-1 z-10 relative py-2 pr-2 sm:py-3 sm:pr-4">
            ...
            {podcast.type && (
              <div
                className={cn(
                  ...
                  !["Warm", "Drive", "Peak"].includes(podcast.type) &&
          ...
          <div className="flex items-center gap-2 ml-auto">
            {!hideLikeButton && (
              <>
                ...
                <div
                  onClick={(e) => e.stopPropagation()}
                  className="transform transition-transform active:scale-95 text-muted-foreground hover:text-red-500"
                >
                  <LikeButton
                    trackId={podcast.id}
        ...
        {/* Genres & Sequence - Bottom */}
        <div className="flex flex-col gap-1 mt-auto">
          {(genreName || parentGenreName || podcast.sequence) && (
            <div className="flex items-center justify-between w-full gap-2 mt-auto">
              ...
              {/* Sequence (Right) */}
              {podcast.sequence && (
                <span className="ml-auto shrink-0 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-muted-foreground border border-white/10 px-1.5 py-0.5 rounded-md bg-white/5">
                  #{podcast.sequence.toString().padStart(3, "0")}
                </span>
              )}
            </div>
          )}
